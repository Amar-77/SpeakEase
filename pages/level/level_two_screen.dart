...
class _LevelTwoScreenState extends State<LevelTwoScreen> with TickerProviderStateMixin {
  ...
  // Words and assets for the speech learning game (can be images or Lottie animations)
  List<String> words = ['pen', 'hen', 'tree', 'bird', 'fish'];
  List<String> wordImages = [
    //'assets/Lottie/pen1.json',      // Lottie animation for "cat"
    'assets/Lottie/1.json',
    'assets/Lottie/hen.json',// Lottie animation for "dog"
    'assets/Lottie/tree.json',      // Lottie animation for "car"
    'assets/Lottie/bird.json',      // Lottie animation for "bug"
    'assets/Lottie/fish.json',     // Lottie animation for "pen" (placeholder)
  ];

  // Custom gradient colors for each word card (unlocked state)
  List<List<Color>> cardGradientColors = [
    [Color(0xfffdfdfd), Color(0xfffdfdfd)],  // Gradient for "pen"
    [Color(0xfffdfdfd), Color(0xfffdfdfd)],  // Gradient for "hen"
  ...
  List<List<Color>> lockedCardGradientColors = [
    [Color(0xff757575), Color(0xff757575)],  // Locked gradient for "pen"
    [Color(0xff757575), Color(0xff757575)],  // Locked gradient for "hen"
  ...
  void _loadProgress() async {
    ...
    List<String>? savedRecognized = prefs.getStringList('level2_wordRecognized');
    List<String>? savedUnlocked = prefs.getStringList('level2_wordUnlocked');
  ...
  void _checkWordPronunciation(String word, bool isPractice) async {
    ...
    if (index != -1) {
      ...
      if (!isPractice && !wordRecognized[index]) {
        ...
        // Save progress
        SharedPreferences prefs = await SharedPreferences.getInstance();
        prefs.setStringList('level2_wordRecognized', wordRecognized.map((e) => e.toString()).toList());
        prefs.setStringList('level2_wordUnlocked', wordUnlocked.map((e) => e.toString()).toList());
